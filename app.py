...
if page == t["history"]:
    ...
    if selected_id:
        ...
        if selected_df.empty:
            ...
            with c1:
                st.markdown(
                    metric_card(
                        ...
                        t["total_result"],
                        "positive" if metrics["net_pnl"] >= 0 else "negative"
                    ),
                    unsafe_allow_html=True
                )
            ...
            with c3:
                st.markdown(
                    metric_card(
                        ...
                        "positive" if metrics["profit_factor"] >= 1 else "negative"
                    ),
                    unsafe_allow_html=True
                )
            ...
            fig_history.update_layout(template="plotly_dark", height=450)
    ...
    try:
        ...
        with u2:
            platform_name = st.selectbox(
                t["platform"],
                ["MT5", "Profit", "TradingView", "NinjaTrader", "Other"]
            )
        ...
        assets = sorted(
            normalized_df["asset"].dropna().astype(str).unique().tolist()
        )

        sides = sorted(
            normalized_df["side"].dropna().astype(str).unique().tolist()
        )
        ...
        with i1:
            st.markdown(
                ...
                <div class="insight-box">
                    <div class="metric-label">{t["best_hour"]}</div>
                    <div class="metric-value positive">{best_hour}h</div>
                    <div class="metric-help">{t["result"]}: {money(best_hour_pnl)}</div>
                </div>
                ''',
                unsafe_allow_html=True
            )
        ...
        with i2:
            st.markdown(
                f'''
                <div class="insight-box">
                    <div class="metric-label">{t["worst_hour"]}</div>
                    <div class="metric-value negative">{worst_hour}h</div>
                    <div class="metric-help">{t["result"]}: {money(worst_hour_pnl)}</div>
                </div>
                ''',
                unsafe_allow_html=True
            )
        ...
        with i3:
            st.markdown(
                f'''
                <div class="insight-box">
                    <div class="metric-label">{t["best_day"]}</div>
                    <div class="metric-value positive">{best_day}</div>
                    <div class="metric-help">{t["result"]}: {money(best_day_pnl)}</div>
                </div>
                ''',
                unsafe_allow_html=True
            )
        ...
        with i4:
            st.markdown(
                f'''
                <div class="insight-box">
                    <div class="metric-label">{t["worst_day"]}</div>
                    <div class="metric-value negative">{worst_day}</div>
                    <div class="metric-help">{t["result"]}: {money(worst_day_pnl)}</div>
                </div>
                ''',
                unsafe_allow_html=True
            )
        ...
        with prop1:
            st.markdown(
                f'''
                <div class="prop-card">
                    <div class="metric-label">{t["target_distance"]}</div>
                    <div class="metric-value neutral">{money(remaining_target)}</div>
                    <div class="metric-help">{t["remaining_to_goal"]}</div>
                </div>
                ''',
                unsafe_allow_html=True
            )
        ...
        with prop2:
            daily_status = "negative" if daily_remaining < 0 else "neutral"
            st.markdown(
                f'''
                <div class="prop-card">
                    <div class="metric-label">{t["daily_dd_remaining"]}</div>
                    <div class="metric-value {daily_status}">{money(daily_remaining)}</div>
                    <div class="metric-help">{t["daily_limit_margin"]}</div>
                </div>
                ''',
                unsafe_allow_html=True
            )
        ...
        with prop3:
            dd_status_prop = "negative" if max_dd_remaining < 0 else "neutral"
            st.markdown(
                f'''
                <div class="prop-card">
                    <div class="metric-label">{t["max_dd_remaining"]}</div>
                    <div class="metric-value {dd_status_prop}">{money(max_dd_remaining)}</div>
                    <div class="metric-help">{t["max_dd_margin"]}</div>
                </div>
                ''',
                unsafe_allow_html=True
            )
        ...
        asset_result = filtered_df.groupby("asset")["net_pnl"].sum().reset_index()
        fig_asset = px.bar(
            asset_result,
            x="asset",
            y="net_pnl",
        ...
        st.markdown(
            f'<div class="footer-note">{t["footer"]}</div>',
            unsafe_allow_html=True
        )
